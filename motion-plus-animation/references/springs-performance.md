...
> Um vocabulário útil nomeia pelo **papel na interface**, não pelo número: algo como
> *snap* (feedback imediato: toggle, aba, hover), *ui* (o padrão: menus, cards, reveals),
> *gentle* (superfícies grandes: seções, sheets), *lively* (comemorativo: badge, contador) e
> *ambient* (fundo contínuo, com `repeat: Infinity`). Assim quem escreve escolhe pela intenção e
> retunar o site inteiro é editar um arquivo.
...
- **Global**: wrap the app once.
  ...
  `reducedMotion` aceita `"user"` (segue a preferência do SO — o padrão sensato), `"always"` e
  `"never"`. Uma boa política intermediária, se o design pede: manter os fades e remover só o
  deslocamento, em vez de desligar tudo.
